...
filedata <- matrix(scan(file=src, what=numeric()), ncol=1000, byrow=TRUE)
...
quant<-round(quantile(data, probs=0.90))
data <- data[data<quant]
printdata(1, data, max(data), "timedelta-1st-delta-90quartile")

data <- filedata[,2]
printdata(1, data, max(data), "timedelta-2nd-delta")
quant<-round(quantile(data, probs=0.90))
data <- data[data<quant]
printdata(1, data, max(data), "timedelta-2nd-delta-90quartile")

data <- filedata[,3]
printdata(1, data, max(data), "timedelta-3rd-delta")
quant<-round(quantile(data, probs=0.90))
data <- data[data<quant]
printdata(1, data, max(data), "timedelta-3rd-delta-90quartile")

data <- filedata[,10]
printdata(1, data, max(data), "timedelta-10th-delta")
quant<-round(quantile(data, probs=0.90))
data <- data[data<quant]
printdata(1, data, max(data), "timedelta-10th-delta-90quartile")
...
write.table(round(apply(filedata, 2, min.entropy), 3), file="boottime-test-timedelta-minentropy.csv", row.names=FALSE, col.names=FALSE)
write.table(round(apply(filedata, 2, shannon.entropy), 3), file="boottime-test-timedelta-shannonentropy.csv", row.names=FALSE, col.names=FALSE)
