...
stats <- function(delta, absolute, probbitset, probbitnotset)
{
  ...
  probbitset <- probbitset[probbitset<1 & probbitset>0]
	...
	tmp <- sprintf("Obere Schranke bei %.0f niederstwertigen Bits: %.2f -- Shannon Entropie: %.2f -- Untere Schranke: %.2f\nGesamtanzahl: %d -- 1st Qu: %.2f -- Median: %.2f -- Mean: %.2f -- 3rd Qu: %.2f", length(probbitnotset), max.entropy(probbitnotset, probbitset), shannon.entropy(delta), min.entropy(delta), length(absolute), quantile(delta, probs=0.25), median(delta), mean(delta), quantile(delta, probs=0.75))
...
drawhistogramdelta <- function(givenname, extension, delta, stat, givenmain)
{
	...
	hist(delta, seq(min(delta)-interval,max(delta)+interval,interval), main=main, prob=FALSE, xlab=stat, ylab="Relative Häufigkeit")
	#Mean value
...
#	else
#	{
		plot(delta, main=main, ylab="Differenzen", xlab=stat, col="1", type=type)
#	}

	dev.off()
}

# sanity checking: list all varable locations whose bit size is larger
